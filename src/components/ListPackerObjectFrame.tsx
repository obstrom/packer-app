...
type ListPackerObjectFrameProps = {
    ...
    setPackerObjects: Dispatch<SetStateAction<Array<Bin>>> | Dispatch<SetStateAction<Array<Item>>>
    handlePackerObjectDelete: (id: string) => void
    className?: string
}

export const ListPackerObjectFrame = ({ headerTitle, packerObjectType, packerObjects, setPackerObjects, handlePackerObjectDelete, className }: ListPackerObjectFrameProps) => {
    const headerTitleAmount = `${headerTitle} (${packerObjects.length})`;

    return (
        <>
            {packerObjects.length === 0 && <div className={`accordion ${className}`}>
            ...
            {packerObjects.length > 0 && <Accordion className={className} defaultActiveKey="0">
